...
SONIOX_API_KEY = "5674d90089ec5a852cd3a1c482aa99297937c9490f0d8d610d288a56095c7f5e"
...
def run_session(
    ...
    while retry_count < max_retries:
        try:
            with connect(SONIOX_WEBSOCKET_URL, close_timeout=30) as ws:
            ...
            if retry_count < max_retries:
                print(f"Connection timeout. Retrying ({retry_count}/{max_retries})...")
...
def main():
    ...
    parser.add_argument("--audio_format", default=None)
    parser.add_argument("--translation", default="none")
    args = parser.parse_args()

    api_key = "5674d90089ec5a852cd3a1c482aa99297937c9490f0d8d610d288a56095c7f5e"
    if api_key is None:
        raise RuntimeError("Missing SONIOX_API_KEY.")

    # Use default audio file if audio_path is not provided
    audio_path = args.audio_path
    if audio_path is None:
        # Get the directory of this script and use a default audio file
        script_dir = os.path.dirname(os.path.abspath(__file__))
        audio_path = os.path.join(script_dir, "..", "assets", "coffee_shop.pcm_s16le")
        
        # Check if default file exists
        if not os.path.exists(audio_path):
            raise RuntimeError(f"Audio file not found: {audio_path}. Please provide --audio_path argument.")

    # Infer audio format from filename if not explicitly provided
    audio_format = args.audio_format
    if audio_format is None:
        if audio_path.endswith(".pcm_s16le"):
            audio_format = "pcm_s16le"
        else:
            audio_format = "auto"

    run_session(api_key, audio_path, audio_format, args.translation)
